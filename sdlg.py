...
from utils import clean_generation, compute_likelihood, generate_text
...
def compute_token_score_ranking(
    ...
    try:
        ...
        target = torch.zeros(len(model_input), dtype=torch.long, device=device_deberta)
    ...
    for word, word_token_indices in zip(
        ...
        if (
            ...
            decoded_word = deberta_tokenizer.decode(
                encoded_answer[word_token_indices]
            ).strip()
            print(f"Error: words do not match ({word.strip()} vs. {decoded_word})")
            return False
...
def generate_semantically_diverse_output_sequences(
    ...
    if not single_word:
        ...
        # Discard SDLG generations if token alignment failed.
        ...
        for i, s in enumerate(sorted_indices):
            ...
            if new_token_idx != args.eos_token_ids:
                ...
                generation_to_add = torch.hstack(
                    [new_input_ids, new_input_ids.new_tensor([new_token_idx])]
                )
                ...
                alternative_generation = {
                    ...
                    "cleaned_generation_ids": [
                        generation_to_add
                        if generation_text == cleaned_generation_text
            ...
            # breaking condition
            if num_added_gens >= args.num_total_generations - 1:
                return results_dict

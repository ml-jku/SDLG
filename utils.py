...
def atomic_pickle_dump(value, path):
    """Write a pickle without exposing a partially written result file."""
    ...
    if model_type_llm is not None:
        ...
        if model_type_llm in [
            ...
            if use_flash_attention:
                ...
                model = (
                    OPTForCausalLM.from_pretrained(opt_path, torch_dtype=torch.bfloat16)
                    if not get_tokenizer_only_llm
                    else None
                )
        else:
    ...
    if model_type_deberta is not None:
        ...
        if model_type_deberta in [
            ...
            deberta_tokenizer = AutoTokenizer.from_pretrained(
                f"microsoft/{model_type_deberta}"
            )
            deberta_model = (
            ...
            raise ValueError(f"model type {model_type_deberta} not supported!")
...
def generate_text(
    ...
    input_ids = (
        input_ids.to(device).reshape(1, -1)
    ...
    if decoding_method == "most_likely":
        ...
        raise ValueError(f"decoding method {decoding_method!r} not supported")

    generation_ids = generation_ids.to("cpu")

    (
        generation_ids_list,
        generation_text_list,
        cleaned_generation_ids_list,
        cleaned_generation_text_list,
    ) = list(), list(), list(), list()
    ...
    for i in range(len(generation_ids)):
        ...
        cleaned_generation_ids_list.append(
            generation_to_add
            if cleaned_generation_text == generation_text
            else tokenizer.encode(
                cleaned_generation_text, add_special_tokens=False, return_tensors="pt"
            )[0]
        )
...
def compute_correctness(
    ...
    if exact_match_metric is not None:
        ...
        for answer in reference_answers:
            results = exact_match_metric.compute(
                predictions=[most_likely_generation_text],
                references=[answer],
                ignore_case=True,
    ...
    if rouge is not None:
        ...
        for answer in reference_answers:
            rouge_results = rouge.compute(
                predictions=[most_likely_generation_text], references=[answer]
            )
        ...
        for incorrect_answer in incorrect_answers:
            rouge_results = rouge.compute(
                predictions=[most_likely_generation_text], references=[incorrect_answer]
            )
    ...
    if bleurt is not None:
        scores_true = max(
            bleurt.compute(
                predictions=[most_likely_generation_text] * len(reference_answers),
        ...
        if len(incorrect_answers) != 0:
            scores_false = max(
                bleurt.compute(
                    predictions=[most_likely_generation_text] * len(incorrect_answers),
...
def compute_likelihood(
    prompt, generation, model, device, compute_cleaned=False, store_logits=False
):
    # This NLL computation follows Kuhn et al. (2023).
    ...
    for i in range(len(generation["generation_ids"])):
        ...
        if compute_cleaned:
            ...
            if (
                torch.equal(cleaned_generation_ids, generation_ids)
                or generation["cleaned_generation_text"][i]
                == generation["generation_text"][i]
            ):
                ...
                # Use negative infinity (zero likelihood) for an empty cleaned generation.
                cleaned_average_neg_log_likelihood = float("-inf")
                cleaned_neg_log_likelihood = float("-inf")
                if store_logits:
                    cleaned_generation_logits = []
            else:
                # Compute NLL following the Transformers perplexity documentation.
                generation_input = torch.hstack([prompt, cleaned_generation_ids]).to(
                    device
                )
                target_ids = generation_input.clone()
                target_ids[: len(prompt)] = -100
                ...
                cleaned_neg_log_likelihood = cleaned_average_neg_log_likelihood * (
                    len(cleaned_generation_ids)
                )

                # compute logits
                if store_logits:
                    cleaned_generation_logits = model_output["logits"][
                        0, len(prompt) - 1 : -1, :
                    ].to("cpu")

            if store_logits:
                list_cleaned_generation_logits.append(cleaned_generation_logits)
            list_cleaned_average_neg_log_likelihood.append(
                cleaned_average_neg_log_likelihood
            )
...
def discard_likelihood_logits(likelihood):
    """Return a shallow copy of a likelihood dictionary without full-vocabulary logits."""
    likelihood_without_logits = dict(likelihood)
    likelihood_without_logits["generation_logits"] = []
    likelihood_without_logits["cleaned_generation_logits"] = []
    likelihood_without_logits["store_logits"] = False
    return likelihood_without_logits
    ...
    if isinstance(average_neg_log_likelihood, list):
        for i in range(len(average_neg_log_likelihood)):
            likelihood_dict = {
                ...
                "cleaned_average_neg_log_likelihood": [
                    cleaned_average_neg_log_likelihood[i]
                ]
                if compute_cleaned
                else [],
                "cleaned_neg_log_likelihood": [cleaned_neg_log_likelihood[i]]
                if compute_cleaned
                else [],
                "cleaned_generation_logits": [cleaned_generation_logits[i]]
                if compute_cleaned and store_logits
                else [],
            }
        ...
        assert (
            not isinstance(neg_log_likelihood, list)
            and not isinstance(cleaned_neg_log_likelihood, list)
            and not isinstance(cleaned_average_neg_log_likelihood, list)
        )
        ...
        likelihood_dict = {
            ...
            "cleaned_average_neg_log_likelihood": [cleaned_average_neg_log_likelihood]
            if compute_cleaned
            else [],
            "cleaned_neg_log_likelihood": [cleaned_neg_log_likelihood]
            if compute_cleaned
            else [],
            "cleaned_generation_logits": [cleaned_generation_logits]
            if compute_cleaned and store_logits
            else [],
        }
...
def prepare_results(
    num_samples, run_key, metric=None, start_sample_id=0, base_path="results"
):
    list_results_dict = []
    dataset_size = 0
    list_correct_labels = []
    ...
    for i in tqdm(range(num_samples), total=num_samples):
        ...
        try:
            ...
            warnings.warn(
                f"Could not read results_dict_{i}.pkl: {error}",
                stacklevel=2,
            )
            continue
        ...
        for generations, likelihoods in zip(
            results_dict[run_key]["generations"], results_dict[run_key]["likelihoods"]
        ):
...
def compute_semantic_pairs(
    ...
    for i, generation_i in enumerate(generations):
        for j, generation_j in enumerate(generations):
            ...
            list_iterations = (
                ["generation_text", "cleaned_generation_text"]
                if compute_cleaned
                else ["generation_text"]
            )
            for k, generation_key in enumerate(list_iterations):
                # Reuse the result if cleaning did not change either generation.
                if (
                    generation_key == "cleaned_generation_text"
                    and generation_i["generation_text"][0]
                    == generation_i["cleaned_generation_text"][0]
                    and generation_j["generation_text"][0]
                    == generation_j["cleaned_generation_text"][0]
                ):
                    semantic_pairs[1][i, j] = semantic_pairs[0][i, j]
                else:
                    if (
                        generation_i[generation_key][0].lower()
                        == generation_j[generation_key][0].lower()
                    ):
                        semantic_pairs[k][i, j] = True
                    else:
                        qa_i = question + " " + generation_i[generation_key][0]
                        qa_j = question + " " + generation_j[generation_key][0]

                        input_sequence = qa_i + " [SEP] " + qa_j
                        encoded_input = deberta_tokenizer.encode(
                            input_sequence, padding=True
                        )
                        prediction = deberta_model(
                            torch.tensor([encoded_input], device=device)
                        )["logits"]
                        predicted_label = torch.argmax(prediction, dim=1)

                        semantic_pairs[k][i, j] = (
                            True if predicted_label != 0 else False
                        )
...
@torch.no_grad()
def compute_batched_semantic_pairs(
    generations,
    deberta_tokenizer,
    deberta_model,
    question,
    ...
    for k, generation_key in enumerate(list_iterations):
        if generation_key == "cleaned_generation_text":
            if False not in [
                g["generation_text"][0] == g["cleaned_generation_text"][0]
        ...
        for batch_i in num_batches:
            for batch_j in num_batches:
                ...
                for generation_i in generations[
                    batch_size * batch_i : batch_size * (batch_i + 1)
                ]:
                    for generation_j in generations[
                        batch_size * batch_j : batch_size * (batch_j + 1)
                    ]:
                        qa_i = question + " " + generation_i[generation_key][0]
                        qa_j = question + " " + generation_j[generation_key][0]
                ...
                index = 0
                for i in range(
                    len(generations[batch_size * batch_i : batch_size * (batch_i + 1)])
                ):
                    for j in range(
                        len(
                            generations[
                        ...
                        semantic_pairs[k][
                            batch_size * batch_i + i, batch_size * batch_j + j
                        ] = True if predicted_labels[index] != 0 else False
                        index += 1
...
def compute_semantic_clusters(
    generations, semantic_pairs, cleaned_semantic_pairs, compute_cleaned=False
):
    semantic_clusters = [
        list(range(0, len(generations))),
        list(range(0, len(generations))),
    ]
    for i, generation_i in enumerate(generations):
        for j, generation_j in enumerate(generations):
            ...
            for k, generation_key in enumerate(list_iterations):
                ...
                # [all] otherwise, check semantic_pairs
                    ...
                    if generation_key == "generation_text":
                        if semantic_pairs[i, j] and semantic_pairs[j, i]:
                            # Note: equivalent to: if predicted_label != 0 and reverse_predicted_label != 0
                            semantic_clusters[k][j] = semantic_clusters[k][i]
                    else:
                        if (
                            cleaned_semantic_pairs[i, j]
                            and cleaned_semantic_pairs[j, i]
                        ):
                            semantic_clusters[k][j] = semantic_clusters[k][i]

    return {
        "semantic_clusters": torch.tensor(semantic_clusters[0]),
        "cleaned_semantic_clusters": torch.tensor(semantic_clusters[1])
        if compute_cleaned
        else torch.tensor([]),
    }
    ...
    for cleaned in list_iterations:
        for nll_key in ["average_neg_log_likelihood", "neg_log_likelihood"]:
            for d in neg_log_likelihoods:
                assert len(d[cleaned + nll_key]) == 1, (
                    "only single likelihoods supported. prepare likelihoods!"
                )
            ...
            log_likelihoods = torch.tensor(
                [
                    -1e12
                    if math.isnan(d[cleaned + nll_key][0])
                    or d[cleaned + nll_key][0] == float("-inf")
                    else -d[cleaned + nll_key][0]
                    for d in neg_log_likelihoods
                ]
            )
            assert torch.all(log_likelihoods <= 0), "likelihood bigger than 1!"
            ...
            for semantic_set_id in torch.unique(
                semantic_difference[cleaned + "semantic_clusters"]
            ):
                aggregated_log_likelihoods.append(
                    torch.logsumexp(
                        log_likelihoods[
                            semantic_difference[cleaned + "semantic_clusters"]
                            == semantic_set_id
            ...
            aggregated_normalized_likelihoods = torch.softmax(
                aggregated_log_likelihoods, dim=0
            )

            if mc_estimate_over_clusters:
                entropy = -torch.sum(aggregated_log_likelihoods, dim=0) / torch.tensor(
                    aggregated_log_likelihoods.shape[0]
                )
            else:
                entropy = -torch.sum(
                    aggregated_log_likelihoods * aggregated_normalized_likelihoods,
                    dim=0,
                )
...
@torch.no_grad()
def compute_semantic_pairs_for_results(
    base_path, model_type, deberta_tokenizer, deberta_model, num_instances, device
):
    for method in ["sdlg", "baseline"]:
        ...
        for i in tqdm(range(0, num_instances)):
            ...
            if (
                len(results_dict[method]["generations"]) == 0
                or len(results_dict[method]["generations"][0]["generation_ids"][0]) == 0
            ):
                if i not in removed_sample_ids:
                    removed_sample_ids.append(i)
                continue
            ...
            prepared_likelihoods = []
            for generations, likelihoods in zip(
                results_dict[method]["generations"], results_dict[method]["likelihoods"]
            ):
                prepared_generation += prepare_generated_text(**generations)
                prepared_likelihoods += prepare_likelihood(**likelihoods)
            ...
            if f"semantic_pairs_{model_type}" not in results_dict[
                method
            ].keys() or results_dict[method][f"semantic_pairs_{model_type}"][
                "semantic_pairs"
            ].shape[0] != len(results_dict[method]["generations"]):
                results_dict[method][f"semantic_pairs_{model_type}"] = (
                    compute_batched_semantic_pairs(
                        generations=results_dict[method]["generations"],
                ...
                atomic_pickle_dump(
                    results_dict,
                    os.path.join(base_path, f"results_dict_{i}.pkl"),
                )

        print(f"{base_path} - {method}: removed_sample_ids: {removed_sample_ids}")


# Backward-compatible alias for the misspelled public name in the initial release.
compute_semantic_paris = compute_semantic_pairs_for_results
